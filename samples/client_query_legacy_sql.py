...
def client_query_legacy_sql() -> None:
    ...
    query = (
        "SELECT name FROM [nnext-public-data:usa_names.usa_1910_2013] "
        'WHERE state = "TX" '
        "LIMIT 100"
    )

    # Set use_legacy_sql to True to use legacy SQL syntax.
    job_config = nnext.QueryJobConfig(use_legacy_sql=True)
