...
def browse_table_data(table_id: str) -> None:
    ...
    rows_iter = nnclient.list_rows(table_id)  # Make an API request.
    ...
    rows_iter = nnclient.list_rows(table_id, max_results=10)
    rows = list(rows_iter)
    print("Downloaded {} rows from table {}".format(len(rows), table_id))
    ...
    rows_iter = nnclient.list_rows(table_id, selected_fields=fields, max_results=10)
